...
#[rstest(spawn_backend as address)]
#[tokio::test]
async fn health_check_works(address: String) {
...
#[rstest(spawn_backend as address)]
#[tokio::test]
async fn v1_users_register_is_200_for_valid_data(address: String) {
    let response = reqwest::Client::new()
...
#[tokio::test]
async fn v1_users_register_is_400_for_missing_data(address: String, #[case] invalid_body: String) {
    let response = reqwest::Client::new()
        .post(&format!("{}/users/register", &address))
        .header("Content-Type", "application/json")
        .body(invalid_body)
